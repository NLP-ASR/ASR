...
class StudentWav2Vec2Model(BaseFairseqModel):
    ...
    def __init__(self, args,change_conv_layers,conv_groups):
        super().__init__()
        self.args = args
        ...
        if self.change_conv_layers:
            self.feature_extractor = ConvFeatureExtractionModel_1(
    ...
    @classmethod
    def build_model(cls, args,change_conv_layers,conv_groups, task=None):
        """Build a new model instance."""

        # make sure all arguments are present
        base_architecture(args)
        #import pdb;pdb.set_trace()
        return cls(args,change_conv_layers,conv_groups)
    ...
    def create_student_model(cls,
        ...
        """
        #import pdb;pdb.set_trace()
        cls.change_conv_layers = change_conv_layers
        cls.conv_groups = conv_groups
        ...
        student_model = cls.build_model(w2v["args"].w2v_args, target_dict,change_conv_layers,conv_groups)
        ...
        for name, param in student_init_model_state.items():
            ...
            if name_in_s_model in student_model_state:
                #import pdb;pdb.set_trace()
                if change_conv_layers and ('feature_extractor.conv_layers' in name_in_s_model) and int(name_in_s_model.split('.')[2]) != 0:
                    in_ch,out_ch,kernel = param.shape
                    #print(name_in_s_model)
                    # take from the beginning(alternate feels off but can be tried)
                    select_channels = list(range(int(out_ch//conv_groups)))
                    
                    student_model_state[name_in_s_model].copy_(param[:,select_channels,:])
                elif change_conv_layers:
...
class ConvFeatureExtractionModel_1(nn.Module):
    def __init__(
        self,
        conv_layers: List[Tuple[int, int, int]],
        dropout: float = 0.0,
        ...
        def block(
            ...
            if is_layer_norm:
                ...
                return nn.Sequential(make_conv(groups), nn.Dropout(p=dropout), nn.GELU())
...
def base_architecture(args):
    #args.change_conv_layers,args.conv_groups  = change_conv_layers,conv_groups 
